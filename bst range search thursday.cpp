	...
	void inorder(node *temp){
       ...
       void rangesearch(node* temp, int s1, int s2){
       	...
       	if(s1<=temp->data && s2>=temp->data){
       	  ...
       	  if(s1<temp->data){
       		n=rcountnode(temp->left,s1,s2);
       	}
       	if(s2>temp->data){
       		n=rcountnode(temp->right,s1,s2);
       	}
                if(s1<=temp->data && s2>=temp->data){
                if(temp==NULL){
                        return n;
		...
		void inorder(){
			...
			cout<<endl;
               ...
               int rcountnode(int s1, int s2){
               	int n=rcountnode(root,s1,s2);
               	return n;
               }
};
...
int main(){
	...
	cout<<n<< endl;
	bst.rsearch(24,56);
            bst.rcountnode(24,56);
            int n1=bst.rcountnode(24,56);
            cout<<n1<<endl;
	}
